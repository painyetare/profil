...
function ReverseService() {
  ...
  return (
    <div 
        ref={serviceRef}
        className='serviceSec w-full bg-primary-main text-primary-text relative overflow-hidden'
    >
        <div className='py-[6vw] sm:py-0 flex flex-col-reverse sm:flex-row items-center justify-between px-[5vw] sm:px-[2.5vw] sm:mb-[4vw]'>
            <div className='reverseLeftDiv w-full sm:w-2/3  sm:px-0 mb-[5vw] sm:mb-0'>
                <div className=' sm:space-y-[4vw] space-y-[12vw] pl-0 sm:pl-[10vw] px-[5vw] '>
                    <div ref={serviceRef2} className='overflow-hidden'>
                        <h4 className='text-[5vw] sm:text-[1.1vw] tracking-wider'>
                            {isRomanian ? '/ Cultura noastră' : '/ Our culture'}
                        </h4>
                    </div>
                    <div className='serviceReversetxt text-[6vw] sm:text-[2.3vw] font-medium tracking-wide sm:leading-[3vw]'>
                        <div className='overflow-hidden'>
                            <h1>{isRomanian ? 'Valorile umane și standardele înalte' : 'Human values and high standards'}</h1>
                        </div>
                        <div className='overflow-hidden'>
                            <h1>{isRomanian ? 'sunt componente de bază ale' : 'are core components of our teams\'s'}</h1>
                        </div>
                        <div className='mb-[12vw] sm:mb-[4vw] overflow-hidden'>
                            <h1>
                                {isRomanian ? 'filosofiei și culturii echipei noastre' : 'philosophy and culture'}
                            </h1>
                        </div>
                        <div className='overflow-hidden'>
                            <h1>
                                {isRomanian ? 'Fie că sunt PM/PO*, designeri sau' : 'Whether PM/PO*, designers or'}
                            </h1>
                        </div>
                        <div className='overflow-hidden'>
                            <h1>
                                {isRomanian ? 'dezvoltatori, cu toții ne străduim pentru' : 'developers, we all strive for'}
                            </h1>
                        </div>
                        <div className='overflow-hidden'>
                            <h1>{isRomanian ? 'excelență în proiectarea unor proiecte' : 'excellence in designing ambitious,'}</h1>
                        </div> 
                        <div className='overflow-hidden'>
                            <h1>{isRomanian ? 'ambițioase și inovatoare.' : 'innovative projects.'}</h1>
                        </div> 
                    </div>
                    <div className='flex items-center justify-center sm:items-start sm:justify-start '>
                        <div className='serviceBtn w-[28vw] h-[28vw] sm:w-[8vw] sm:h-[8vw] rounded-full border-[.8px] border-[#d9d9d9] flex items-center justify-center'>
                            <h3 className='sm:text-[.9vw]'>{isRomanian ? 'Valorile noastre' : 'Our values'}</h3>  
                        </div>  
                    </div>  
                </div>  
            </div>
